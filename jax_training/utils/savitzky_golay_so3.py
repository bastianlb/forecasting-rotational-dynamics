...
from .so3 import map_to_lie_algebra, rodrigues, log_map, symmetric_orthogonalization, ddexp_so3
...
def _so3_filter_solve(A: jnp.ndarray, b: jnp.ndarray, R_center: jnp.ndarray, p: int) -> Tuple[jnp.ndarray, jnp.ndarray]:
...
@partial(jax.jit, static_argnums=(3,))
def _so3_filter_solve_weighted(A_weighted: jnp.ndarray, b_weighted: jnp.ndarray, R_center: jnp.ndarray, p: int) -> Tuple[jnp.ndarray, jnp.ndarray]:
...
def _so3_filter_core(
    ...
    if weight is not None:
        ...
        R_smoothed, rho = _so3_filter_solve_weighted(A_weighted, b_weighted, R_center, p)
    else:
...
def so3_filter(
    ...
    return so3_savitzky_golay_filter(
        R, t, p, weight=weight, return_omega=return_omega, return_coefficients=return_coefficients
    )


# Vectorized version for batch processing
so3_filter_batch = jax.vmap(so3_savitzky_golay_filter, in_axes=(0, None, None, 0, None, None))
...
class SO3PolynomialPath(diffrax.AbstractPath):
    ...
    def __init__(self, R: jnp.ndarray, t: jnp.ndarray, p: int, 
                 weight: Optional[jnp.ndarray] = None, second_order: bool = False):
        ...
        if R.ndim != 4:
            raise ValueError(f"Expected R to have shape (batch_size, n_points, 3, 3), got {R.shape}")
        ...
        if first_deriv_coeffs:
            first_deriv_coeffs = jnp.stack(first_deriv_coeffs, axis=-2)  # (batch_size, p, 3)
        else:
        ...
        if second_deriv_coeffs:
            second_deriv_coeffs = jnp.stack(second_deriv_coeffs, axis=-2)  # (batch_size, p-1, 3)
        else:
        ...
        powers_second = jnp.arange(max(1, p-1), dtype=jnp.float32)  # [0, 1, ..., p-2] or [0] if p=1
    ...
    def _compute_first_order_derivative_optimized(self, t_rel):
        ...
        result = _first_order_vmap(self.phi_coeffs, self.R_center, 
                                  self.first_deriv_coeffs, self.second_deriv_coeffs,
                                  t_rel, self.powers_phi, self.phi_normalizers, self.powers_first)
    ...
    def _compute_second_order_derivative_optimized(self, t_rel):
        ...
        result = _second_order_vmap(self.phi_coeffs, self.R_center,
                                   self.first_deriv_coeffs, self.second_deriv_coeffs,
                                   t_rel, self.powers_phi, self.phi_normalizers, 
                                   self.powers_first, self.powers_second)
    ...
    def _evaluate_single(self, t_val):
        ...
        def _fast_evaluate_single_element(phi_coeffs_single, R_center_single):
            ...
            phi = jnp.sum(phi_coeffs_single * (t_powers_phi / self.phi_normalizers)[:, None], axis=0)  # (3,)
    ...
    def derivative(self, t, left=True, order=1):
        ...
        if order == 2 and not self.second_order:
            raise ValueError("Second-order derivatives requested but second_order=False in constructor")
...
@jax.jit
def _compute_first_order_derivative_element(phi_coeffs_single, R_center_single, first_coeffs, second_coeffs,
                                          t_rel, powers_phi, phi_normalizers, powers_first):
...
@jax.jit
def _compute_second_order_derivative_element(phi_coeffs_single, R_center_single, first_coeffs, second_coeffs,
                                           t_rel, powers_phi, phi_normalizers, powers_first, powers_second):
...
_first_order_vmap = jax.jit(jax.vmap(_compute_first_order_derivative_element, 
                                    in_axes=(0, 0, 0, 0, None, None, None, None)))
_second_order_vmap = jax.jit(jax.vmap(_compute_second_order_derivative_element,
